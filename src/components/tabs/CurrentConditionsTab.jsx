...
import { Thermometer, Wind, Droplets, Zap, Sunrise, Sunset, Maximize, Radio, CloudRain, Sun, Volume2, VolumeX, ChevronDown, ChevronUp } from 'lucide-react';
...
const generateWeatherSummary = (current, daily, night, alerts, units) => {
  ...
  if (Math.abs(temp - feelsLike) >= 5) {
    if (feelsLike < temp) {
      summary += `Wind chill makes it feel like ${fmtTemp(feelsLike, units)}. `;
    } else {
      summary += `Heat index makes it feel like ${fmtTemp(feelsLike, units)}. `;
    }
  }
  ...
  if (maxWind > 0) {
    let windDesc;
    if (maxWind >= 25) {
      windDesc = `Strong winds from the ${windDir} up to ${fmtWind(maxWind, units)}`;
    } else if (maxWind >= 15) {
      windDesc = `Breezy with winds from the ${windDir} up to ${fmtWind(maxWind, units)}`;
    } else {
      windDesc = `Light winds from the ${windDir} around ${fmtWind(maxWind, units)}`;
    }
    if (gustMax > maxWind + 5) {
      windDesc += `, gusting to ${fmtWind(gustMax, units)}`;
    }
    summary += windDesc + '. ';
  }

  // High/Low
  summary += `Today's high near ${fmtTemp(high, units)} with a low of ${fmtTemp(low, units)}.`;
...
const StatCard = ({ children, className = '' }) => (
  <div className={`p-3 bg-black/20 rounded-lg border border-cyan-700 flex flex-col items-center hover:border-cyan-400 hover:bg-black/30 hover:shadow-neon-md hover:scale-105 transition-all duration-300 cursor-default ${className}`}>
    {children}
  </div>
);
...
const CurrentConditionsTab = ({ current, daily, hourly, night, isWeatherLoading, alerts, aqiData, location, units }) => {
  ...
  const handleNarration = () => {
    ...
    const text = getNarrationText(SCREENS.CONDITIONS, {
      weatherData: { current, daily, hourly },
      alerts,
      location,
    });
    const utterance = speakNarration(text);
    if (utterance) {
      setIsSpeaking(true);
    ...
    <TabPanel title="CURRENT CONDITIONS">
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        <div className="flex-grow p-4 rounded-lg border-2 border-cyan-600 bg-black/30">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-lg text-cyan-300 font-bold flex items-center gap-2">
              <Radio size={18} /> FORECAST SUMMARY
            </h3>
            <button
              onClick={handleNarration}
              className={`p-1.5 rounded transition-colors ${
                isSpeaking ? 'bg-cyan-500 animate-pulse' : 'bg-cyan-900/50 hover:bg-cyan-700'
              }`}
              title={isSpeaking ? 'Stop reading' : 'Read forecast aloud'}
            >
              {isSpeaking
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-white font-vt323 text-lg">
        <StatCard>
          ...
          <span className="font-bold">{fmtTemp(currentData.apparent_temperature ?? currentData.temperature_2m ?? 0, units)}</span>
        </StatCard>
        <StatCard>
          ...
          <span className="font-bold">{fmtWind(currentData.wind_speed_10m || 0, units)} {degreeToCardinal(currentData.wind_direction_10m || 0)}</span>
        </StatCard>
        <StatCard>
          <Zap size={20} className="text-cyan-400" />
          <span className="text-sm text-cyan-300">WIND GUSTS</span>
          <span className="font-bold">{fmtWind(currentData.wind_gusts_10m || 0, units)}</span>
        </StatCard>
        <StatCard>
        ...
        <StatCard>
          ...
          <span className="font-bold">{fmtTemp(dailyData.max || 0, units)} / {fmtTemp(dailyData.min || 0, units)}</span>
        </StatCard>
        <StatCard>
        ...
        <StatCard className={`${uvInfo.bg} ${uvInfo.border}`}>
        ...
        {/* AQI - spans 3 columns to fill the row after SUNSET */}
        <div className={`p-3 rounded-lg border flex flex-col items-center col-span-3 hover:shadow-neon-md hover:scale-105 transition-all duration-300 cursor-default ${
      ...
      {/* Collapsible details (dew point + pressure) */}
      <div className="mt-3">
        <button
          onClick={() => setShowDetails(d => !d)}
          className="flex items-center gap-1 text-sm text-cyan-600 hover:text-cyan-400 transition-colors"
        >
